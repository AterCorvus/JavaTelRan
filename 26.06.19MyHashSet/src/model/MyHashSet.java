...
public class MyHashSet<T> implements IHashSet<T>{
	...
	public boolean add(T data) {
		if((1 << n) * loadFactor < size)
			duplShelves();
		
		ArrayList<T> alt = hs.get(getNumShelf(data));
		if (alt.contains(data))
			return false;
	...
	private void duplShelves() {
		
		n++;
		int initSize = hs.size();
